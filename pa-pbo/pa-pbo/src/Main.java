...
public class Main {
    ...
    static KaryawanManager karyawanManager = new KaryawanManager();

    public static void main(String[] args) {
        fallbackUserData();
        karyawanManager.initSampleData();
    ...
    static void fallbackUserData() {
        databaseUser.clear();
        databaseUser.add(new User("admin_utama", "admin123"));
        databaseUser.add(new User("penjaga_1", "penjaga111"));
    }
    ...
    static void register() {
        ...
        if ("admin".equalsIgnoreCase(u)) {
            System.out.println("❌ Username 'admin' tidak bisa dipakai.");
            return;
        }
        for (User user : databaseUser) {
    ...
    static void tampilkanMenuAdmin() {
        System.out.println("\n🛠️ << Dashboard Admin >> [Admin: " + currentUser.getUsername() + "]");
        System.out.println("1. Tambah Karyawan");
        System.out.println("2. Lihat Karyawan");
        System.out.println("3. Update Karyawan");
        System.out.println("4. Hapus Karyawan");
        System.out.println("5. Logout & Keluar");
    }

    static boolean isAdmin() {
        return currentUser != null && "admin".equals(currentUser.getUsername()) && "admin123".equals(currentUser.getPassword());
    }
    ...
    static void adminDashboard() {
        ...
        while (programBerjalan) {
            ...
            switch (pilihan) {
                case 1: karyawanManager.tambahKaryawan(scanner); break;
                case 2: karyawanManager.lihatKaryawan(); break;
                case 3: karyawanManager.updateKaryawan(scanner); break;
                case 4: karyawanManager.hapusKaryawan(scanner); break;
                case 5:
                    System.out.println("👋 Terima kasih, admin! Program ditutup.");
                    programBerjalan = false;
                    break;
                default: System.out.println("⚠️ Pilihan tidak valid.");
            }
